...
# 		# inq.save()
# 		return render(request,'home.html',{'info':"Today's attendance is already updated can't update again..."})

def up_attend(request):
	id = request.POST.getlist('att')
	pdate = date.today()
	dt = present.objects.filter(adate=pdate)
	if(dt):
		return render(request,'home.html',{'info':"Today's attendance is already updated can't update again..."})
	else:
		k = present.objects.get(adate=pdate)
		k.s_ids = id
		k.save()
		return render(request,'home.html',{'info':"Today's attendance is updated successfuly!!! "})
